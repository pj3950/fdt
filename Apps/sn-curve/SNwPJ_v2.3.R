...

{
  ...
  if (any(RO=="RO"))   # Run-outs?
  {
    ...
    ## bestäm en sammanvägd standardavvikelse baerad på förkunskap
  ...
  pi<-matrix(ncol=3,nrow=length(ref$S))
  pi[,1]<-out$estimate[1]+out$estimate[2]*log(ref$S)
  if (int=="conf") rot<-sqrt(rot_man^2-1) else rot<-rot_man
  ...
  {
    limx<-c(min(pi[,2]),max(pi[,3]))
    limy<-sort(c(ref$S[which.min(pi[,2])],ref$S[which.max(pi[,3])]))
...
# plot(data$N[fail],data$S[fail],log="xy",pch=16,xlim=exp(limx),ylim=limy,lwd=2,xlab=xlabel,ylab=ylabel,main=title)
  ...
  if(plottype == "l" | plottype == "b") {
    ...
    if (no_int)
    {
      lines(exp(pi[,1]),ref$S,lwd=2,col="blue")
      
      #Om vi vill ha nedre två-sigmaintervall
      if(DNVplot)
        lines(exp(pi[,1]-2*out$estimate[3]),ref$S,lwd=2,lty="dashed",col="blue")
    }
    
    #annars plottar vi också dem med den typ och konfidensgrad som angetts
    else
    {
#      lines(exp(pi[,3]),ref$S,lwd=1,lty=2,col="blue")
      lines(exp(pi[,3]),ref$S,lwd=2,lty=1,col="blue")
      lines(exp(pi[,2]),ref$S,lwd=2,lty=2,col="blue")
      lines(exp(pi[,1]),ref$S,lwd=2,col="blue")
    }#else
  ...
  if (!is.na(Npred))
  {
    ...
    Strength[1]<-exp(approx(pi[,1],log(ref$S),log(Npred))$y)
    Strength[2]<-exp(approx(pi[,2],log(ref$S),log(Npred))$y)
    Strength[3]<-exp(approx(pi[,3],log(ref$S),log(Npred))$y)
    StrengthDNV<-exp(approx(pi[,1]-2*out$estimate[3],log(ref$S),log(Npred))$y)
    #Om vi vill rita ut linjer
    if(plottype == "l" | plottype == "b") {
      #och ritar ut det hela
...
#  result.Strength.int<-signif(rev(Strength),3)
  ...
  result.Strength.int<-signif(Strength[2:3],3)

SNw<-function(S, N, RO, title="", ylabel="", xlabel="", formel=TRUE, int="pred", conf.level=0.95, conf.level.par=conf.level,
              beta_low=NA, beta_high=NA, S_prior=NA, S_prior_high=NA, Nstrength=2e6, 
              pred.S.from.N=2e6, pred.N.from.S=NA,
#              preds_low=NA,preds_high=NA,
              Srange=NA, Nrange=NA,
              plottype="b",
...
#conf.level.par täckningssannolikhet för parametrar  
...

{
  ...
  # Vilket sigma ska man använda vid förkunskap av beta???
  ...
  # Calculate loss function as square-sum of residuals
  Q <- sum((p$residuals)^2) 
...
#    sigma<-out$estimate[3]*origN/(origN-2) # Bias correction of std
    ...
    sigma <- out$estimate[3]*sqrt(origN/nyeq) # Bias correction of std
    ...
    # Calculate loss function as Likelihood
    Q <- out$minimum  # Value of Likelihood 
  ...
  for (i in 1:length(ref$S)) 
    rot_man[i] <- sqrt(1+1/length(data$S)+(log(ref$S[i])-mean(log(data$S[1:len])))^2/denom)
...
#  plot(NA,NA,log="xy",pch=16,xlim=exp(limx),ylim=limy,lwd=2,xlab=xlabel,ylab=ylabel,main=title)
  ...
  if(plottype == "l" | plottype == "b") {
    ...
    lines(c(exp(limx[1]),Nstrength,Nstrength),c(Se,Se,0.1*limy[1]), col="black", lwd=2, lty="dotted")
    ...
    if (length(predindex)>0)
    {
      ...
      for (i in predindex)
      {
        ...
        lines(c(1,exp(qi[index,2])),predS[i]*c(1,1),col="black",lwd=1)
        lines(exp(qi[index,2])*c(1,1),c(1,predS[i]),col="black",lwd=1)
        lines(c(1,exp(qi[index,1])),predS[i]*c(1,1),col="black",lwd=1)
        lines(exp(qi[index,1])*c(1,1),c(1,predS[i]),col="black",lwd=1)
  ...
  ####################################################################
  ####################################################################
  ##Om vi vill ha en styrka för en viss livslängd... pred.S.from.N
 ...
 predN <- pred.S.from.N
  ...
  if (!is.na(predN))
  {
    ...
    Strength[1] <- exp(approx(qi[,1],log(ref$S),log(predN))$y)
    Strength[2] <- exp(approx(qi[,2],log(ref$S),log(predN))$y)
    Strength[3] <- exp(approx(qi[,3],log(ref$S),log(predN))$y)
    StrengthDNV <- exp(approx(qi[,1]-2*out$estimate[3],log(ref$S),log(predN))$y)
    #Om vi vill rita ut linjer
    if(plottype == "l" | plottype == "b") {
      #och ritar ut det hela
      lines(c(1,predN),Strength[1]*c(1,1),col="black",lwd=2)
      if (DNVplot)
      {
        lines(c(1,predN),StrengthDNV*c(1,1),col="black",lwd=2)
        lines(c(1,1)*predN,c(par("yaxp")[1]/10,StrengthDNV),col="black",lwd=2)
      }
      else
      {
        lines(c(1,1)*predN,c(par("yaxp")[1]/10,Strength[2]),col="black",lwd=2)
        lines(c(1,predN),Strength[2]*c(1,1),col="black",lwd=2)
      }
      lines(c(1,1)*predN,c(par("yaxp")[1]/10,Strength[1]),col="black",lwd=2)
  ...
  #Vi beräknar prediktionsgränser med interpolering ... för predN
  Strength95<-0
  Strength95[1]<-exp(approx(pi95[,1],log(ref$S),log(predN))$y)  # median
  Strength95[2]<-exp(approx(pi95[,2],log(ref$S),log(predN))$y)  # lower pred.lim.

  
  ##Vi beräknar konfidensinterval för konfidensgrad
  
  p.par <- 1-(1-conf.level.par)/2
  
  ci95<-matrix(ncol=3,nrow=length(ref$S))
  ci95[,1]<-out$estimate[1]+out$estimate[2]*log(ref$S)
  rot<-sqrt(rot_man^2-1)
  ci95[,2]<-ci95[,1]-out$estimate[3]*rot*qt(p.par,nyeq+ny_prior)
  ci95[,3]<-ci95[,1]+out$estimate[3]*rot*qt(p.par,nyeq+ny_prior)
  
  #Vi beräknar konfidensintervall för FAT-value, Se (med interpolering)
  ciFAT<-0
  ciFAT[1]<-exp(approx(ci95[,2],log(ref$S),log(Nstrength))$y)
  ciFAT[2]<-exp(approx(ci95[,3],log(ref$S),log(Nstrength))$y)
  
  #Vi beräknar konfidensintervall för skadeexponent, k (med interpolering)
  ci.k <- 0
  k.pm <- out$estimate[3]/sqrt(denom)*qt(p.par,nyeq+ny_prior)
  ci.k <- -out$estimate[2]+k.pm*c(-1,1)
  
  #Alla inressanta resultat bestäms och läggs i en vektor
  
  result.Se<-signif(Se,3)
  result.k<-signif(-out$estimate[2],6)
  result.k.pm <- k.pm
  result.k.int <- signif(ci.k,6)
